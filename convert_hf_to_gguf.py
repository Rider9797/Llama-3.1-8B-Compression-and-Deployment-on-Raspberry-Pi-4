...
class LlamaModel(Model):
    ...
    def set_gguf_parameters(self):
        # --- PRUNED MODEL FIX: Detect Variable MLP Sizes ---
        # 1. Initialize a list to hold the size of EVERY layer
        #    (We assume 32 layers for 8B, but we'll use block_count to be safe)
        self._ffn_dims = [0] * self.block_count
        
        logger.info("SCANNING MODEL FOR VARIABLE MLP SIZES...")
        
        # 2. Iterate through all tensors to find the REAL size of each layer
        #    This allows us to handle 'jagged' pruning where some layers are small and others are big.
        detected_sizes = {}
        
        for name, data in self.get_tensors():
            # We look for the gate_proj weight to determine the intermediate size
            # name format is usually: model.layers.5.mlp.gate_proj.weight
            if "mlp.gate_proj.weight" in name:
                # Extract the layer index (bid)
                # Split by dot and find the number after 'layers'
                parts = name.split('.')
                try:
                    layer_idx = int(parts[2]) # "layers.5" -> 5
                    
                    # shape[0] is the intermediate size (e.g., 14336 or 10752)
                    current_size = data.shape[0]
                    detected_sizes[layer_idx] = current_size
                    
                except (IndexError, ValueError):
                    continue

        # 3. Fill the list. If a layer wasn't found (rare), fallback to config default
        default_size = self.hparams.get("intermediate_size")
        for i in range(self.block_count):
            if i in detected_sizes:
                self._ffn_dims[i] = detected_sizes[i]
            else:
                logger.warning(f"Could not detect size for layer {i}, using default {default_size}")
                self._ffn_dims[i] = default_size

        logger.info(f"Detected MLP sizes (first 5): {self._ffn_dims[:5]}")
        logger.info(f"Detected MLP sizes (pruned section): {self._ffn_dims[5:10]}")

        # --- END FIX ---

        # NOW WRITE THE PARAMETERS
        # Ideally, we pass the LIST (_ffn_dims) to add_feed_forward_length
        # This tells llama.cpp that every layer has a specific size.
