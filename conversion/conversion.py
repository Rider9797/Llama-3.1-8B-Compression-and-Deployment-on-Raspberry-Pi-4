...
def run_folder_conversion():
    ...
    print(f"🔍 Checking for pruned model at: {RAW_DIR}")
    if not os.path.exists(f"{RAW_DIR}/pytorch_model.bin"):
        print("❌ Error: pruned_model.bin not found!")
        ...
        print("✅ Found raw checkpoint from pruning script.")
    ...
    if not os.path.exists(f"{CLEAN_DIR}/config.json"):
        ...
        try:
            ...
            if 'tokenizer' in ckpt:
                ...
                print("⚠️ No tokenizer found in checkpoint. Checking folder artifacts...")
                ...
                for f in os.listdir(RAW_DIR):
                    if "token" in f or "vocab" in f:
                        ...
                        files_copied += 1
                if files_copied > 0:
                    print(f"   ✅ Copied {files_copied} tokenizer files manually.")
                else:
                    print("   ⚠️  Warning: No tokenizer files found. GGUF conversion might fail.")
