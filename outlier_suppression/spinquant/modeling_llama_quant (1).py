...
class LlamaMLP(nn.Module):
    def __init__(self, config, intermediate_size=None): # Add intermediate_size here
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        # Use the passed size if available, otherwise use config
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        # --- START OF FIX ---
        # Define which layers are pruned
        if 4 <= layer_idx <= 27:
            layer_int_size = 10752
        else:
            layer_int_size = 14336
            
        self.mlp = LlamaMLP(config, intermediate_size=layer_int_size)
        # --- END OF FIX ---
