...
def prepare_model(args, model):
    ...
    # 1. Rotate the weights
    fuse_norm_utils.fuse_layer_norms(model)
    
    # --- TA INSTRUCTION: Remove offline rotation in W_down (R4) ---
    ...
    # --- TA INSTRUCTION: Remove online R4 (The block that caused the Power of 2 crash) ---
    """
    for name in qlayers:
    ...
    # --- TA INSTRUCTION: Switch off R3 rotation (Comment out QKRotation wrapper) ---
    if args.k_bits < 16:
        if args.k_pre_rope:
            raise NotImplementedError("Pre-RoPE quantization is not supported yet!")
        else:
            # We skip this block as per TA instructions to disable R3
            pass
